...
switch (_lockState) do
{
	...
	{
		R3F_LOG_mutex_local_verrou = true;
		//The Scotsman - Reduce Unlock time
		//_totalDuration = if (_object getVariable ["ownerUID", ""] == getPlayerUID player) then { 5 } else { 1800 }; // Allow owner to unlock quickly
		_totalDuration = if (_object getVariable ["ownerUID", ""] == getPlayerUID player) then { 3 } else { 3 }; // Allow owner to unlock quickly
