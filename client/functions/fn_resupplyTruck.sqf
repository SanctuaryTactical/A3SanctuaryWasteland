...
#define RESUPPLY_TRUCK_DISTANCE (15 max (sizeOf typeOf _vehicle * 0.75)) // this must match the addAction condition in fn_setupResupplyTruck.sqf
...
_resupplyThread = [_vehicle, _unit] spawn
{
	...
	_checkAbortConditions =
	{
		...
		call
		{
			...
			_checkCondition = {{alive _x && {_x getVariable ["A3W_resupplyTruck", false]}} count (_vehicle nearEntities [["AllVehicles", "B_CargoNet_01_ammo_F", "Land_RepairDepot_01_green_F"], RESUPPLY_TRUCK_DISTANCE]) == 0};
			if (call _checkCondition) exitWith
			{
