...
switch (true) do {
	...
	case (ITEM_COUNT(MF_ITEMS_EXTINGUISHER) <= 0 || {round getNumber (configFile >> "CfgVehicles" >> typeOf player >> "engineer") > 0 && !("ToolKit" in items player)}): {_error = ERR_NO_EXTINGUISHERS};
};
_error;
