...
_setupObjects =
{
	...
	{ // you don't really need this line, delete it.
		hint format ["TH;E STEALCAR %1\nWAS DESTROYED!\nMISSION FAILED!",_vehName]; // you don't really need this line, delete it.
	};
	_stealCar = _missionVehicle;
	
	waitUntil {(!alive _stealCar)}; // waits for the vehicle to not exist anymore (destroyed or successful mission)
