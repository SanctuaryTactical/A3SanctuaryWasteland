...
private ["_vehChoices", "_convoyVeh", "_veh1", "_veh2", "_veh3", "_createVehicle", "_vehicles", "_leader", "_speedMode", "_waypoint", "_vehicleName", "_vehicleName2", "_numWaypoints", "_Boxes", "_Box", "_box1", "_Box2", "_box3"];

_setupVars =
{
	_missionType = "Submarine Patrol";
	_locationsArray = SubmarineConvoyPaths;
};
...
_setupObjects =
{
	...
	_vehChoices =
	[
		["HAFM_Virginia", "B_Heli_Transport_01_F", "HAFM_PBoat"],
		["HAFM_Yasen", "O_Heli_Light_02_F", "HAFM_CB90"],
		["HAFM_209", "I_Heli_light_03_F", "HAFM_GunBoat"],
		["HAFM_214", "I_Heli_light_03_F", "HAFM_MisBoat"]		
	];
	...
	_createVehicle =
	{
		...
		switch (true) do
		{
			case (_type isKindOf "HAFM_Virginia" || _type isKindOf "HAFM_Yasen" || _type isKindOf "HAFM_209" || _type isKindOf "HAFM_214"  || _type isKindOf "HAFM_MisBoat" || _type isKindOf "HAFM_PBoat"):
			{
				...
				if (missionDifficultyHard) then
				{
					...
					_soldier = [_aiGroup, _position] call createRandomSoldierC;
					_soldier moveInTurret [_vehicle, [2]]; // commanderseat - front gunner
					_soldier = [_aiGroup, _position] call createRandomSoldierC;
					_soldier moveInTurret [_vehicle, [3]]; // commanderseat - front gunner
					_soldier = [_aiGroup, _position] call createRandomSoldierC;							
				};
	...
	_missionHintText = format ["A <t color='%3'>%1</t> CLASS submarine is patrolling the coasts, escorted by a <t color='%3'>%2</t>.<br/>Intercept them and recover $1 million dollars!", _vehicleName, _vehicleName2, waterMissionColor];
...
_successExec =
{
	...
	_veh1 = selectrandom ["C_Boat_Civil_01_F"];
	_lastPos set [2, 200];
	_Box = createVehicle [_veh1, _lastPos, [], 0, "None"];
	...
	_successHintMessage = "The submarine patrol has been stopped, , an airdrop of a small craft has been dropped. Navigate to the boat and collect your reward!<br/>Please destroy the light craft once looted";
};
